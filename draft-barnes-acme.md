...
When the identifier being validated is a domain name, the client can prove control of that domain by provisioning resource records under it.   The DNS challenge requires the client to provision a TXT record containing a validation token under a specific validation domain name.
...

{
  ...
  "serverPublicKey": { "kty": "EC", ... }
}

~~~~~~~~~~

In response to this challenge, the client computes the value it will provision to the DNS as the MAC over a JSON object.  The validation object covered by the MAC has the following fields:
...
The to compute the MAC key, the client generates an ECDH key pair and uses it with the serverPublicKey value in the challenge to generate a 256-bit shared secret according to the algorithm defined in {{key-agreement}}.
...
The response to a DNS challenge provides the ECDH key that the client generated and the HMAC-based JWS object.

type (required, string):
: The string "dns"

clientPublicKey (required, JWK):
: The ECDH key generated by the client
